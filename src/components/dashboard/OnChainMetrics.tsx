    ...
    <div className="rounded-2xl border border-border bg-card p-4 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-primary/50 to-transparent">
        <div 
          ...
          style={{ width: `${((60 - countdown) / 60) * 100}%`, transition: 'width 1s linear' }}
        />
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-primary px-1 py-0.5 bg-primary/10 rounded font-medium">
            LIVE
          </span>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="p-3 rounded-xl bg-secondary/50">
          ...
          <div className="text-xs text-muted-foreground">
            {metrics.hashRate > 0 ? `${(metrics.hashRate / 1e18).toFixed(2)} EH/s hash rate` : 'network activity'}
          </div>
        </div>
      </div>
